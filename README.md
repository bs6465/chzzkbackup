...
- 플레이어의 채팅 사이드패널은 영상 시점과 자동 동기화되며 유형 필터, 검색, 클릭 탐색, 시간 단위가 포함된 타임라인과 브라우저 전체 영상에 적용되는 ±60초 딜레이 보정을 지원합니다.
- 녹화 중 방송과 다시보기에서 메모가 있는 북마크를 여러 개 저장하고, 영상별 전체 시간 보정·개별 시간 및 내용 수정·삭제·클릭 재생을 지원합니다.
...
11. 채팅 검색창 옆의 `−0.5초`, `+0.5초`, `초기화` 버튼으로 영상과 채팅의 시차를 보정합니다.
12. 녹화 중 행의 `현재 시점 체크`로 생방송 북마크를 남기거나, 다시보기에서 `현재 위치 북마크` 또는 `B` 키로 북마크를 추가합니다.
13. 다시보기 북마크의 시간을 직접 수정하거나 현재 재생 위치로 바꾸고, 영상별 `−0.5초`, `+0.5초`, `초기화`로 전체 시차를 보정합니다.
